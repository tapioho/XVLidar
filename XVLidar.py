...
class XVLidar:
    """ A class for using the XV LiDAR controller from www.getsurreal.com in a more object oriented way.
    ...
    def scan(self, duration=1):
        ...
        startTime = time.time()  # Starting time for tracking how long the scan has been going on for
    ...
    def motorOff(self):
        # Sends the command 'MotorOff' to the serial port, which stops spinning the lidar
        self.sendCommand("MotorOff")

    def motorOn(self):
        # Sends the command 'MotorOn' to the serial port, which starts spinning the lidar 
        self.sendCommand("MotorOff")
    ...
    def reset(self):
        # Resets the LiDAR and restores the original configuration by sending the command 'ResetConfig' to the serial port
        self.sendCommand("ResetConfig")

    def sendCommand(self, cmd):
        # Sends a command in the correct form through the serial port into the LiDAR
